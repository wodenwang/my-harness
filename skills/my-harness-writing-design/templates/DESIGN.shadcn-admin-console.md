...
| 新增 / 编辑面板 | 按复杂度选择 `Dialog`、`Sheet` 或独立详情页 + form components |
| 确认操作 | `AlertDialog` |
| 状态展示 | `Badge` |
...
- 一级导航稳定。
- 二级导航必须体现树形层级，不用扁平列表伪装。
- 当前页面必须高亮。
- 当前二级页面应能看出所属一级分组。
- 支持桌面展开 / 收缩。
- 收缩态使用 icon + tooltip + `aria-label`。
- 移动端使用 `Sheet` / Drawer 承载导航。
- 导航状态应尽量与路由同步，刷新后当前态保持正确。

## 7. 按钮规范

后台系统按钮必须优先服务于清晰操作，不用缩写文案或换行文案来硬塞布局。

默认规则：

- 除列表页和空间较窄场景外，按钮一律使用 icon + 文字。
- 列表页允许使用纯 icon 按钮，适用于表格行操作、工具栏快捷操作、操作列空间有限、刷新/列设置/更多操作等高频紧凑控件。
- 非列表页中，如果容器宽度确实不足或需要紧凑设计，也可以使用纯 icon 按钮。
- 纯 icon 按钮必须有 `aria-label`。
- 纯 icon 按钮必须有 `title`、`Tooltip` 或等价说明。
- 图标含义必须稳定、常见、可理解。
- 按钮文案不允许换行。
- 如果按钮必须换行才放得下，视为空间较窄场景：优先改为纯 icon 按钮，或调整布局把次要操作收入 `DropdownMenu`。
- 输入框 + 按钮组合空间不足时，可以整组换行，但不能把单个按钮压成多行。
- 长文案按钮在窄屏下应改短文案、纯 icon + tooltip、菜单收纳或重新布局。
...
| 页面主操作 | icon + 文字 |
| 表单提交 / 取消 | icon + 文字 |
| Dialog / Sheet footer | icon + 文字 |
| 列表页行操作 | 纯 icon 或 `DropdownMenu` |
| 列表页刷新 / 列设置 / 密度切换 | 纯 icon |
| 空间较窄的工具栏 | 纯 icon 或 `DropdownMenu` |
| 危险操作 | icon + 文字；列表行内可用纯 icon，但必须有确认和可访问标签 |

表格操作列要求：

- 使用稳定宽度。
- 使用 icon button group。
- 按钮尺寸、间距、顺序一致。
- 危险操作保留危险色和确认流程。
- 不因按钮导致行高异常。
...
筛选项过多时，次要条件放入 `Collapsible`、`Popover` 或高级筛选，不要把首屏挤满。
...
- 默认只展示主 ID、标题、状态、时间、操作和少量关键字段。
- 描述、长 URL、长 JSON、长错误信息、长权限点清单等放入详情，不强塞列表。
- 状态列、时间列、操作列必须稳定宽度。
- 长标识符必须截断、tooltip 或复制，不能撑破表格。
- 技术标识符不能裸值展示，应带字段名，例如 `user_id: xxx`、`request id: xxx`。
...
Dialog 适合新增、编辑、确认、简单表单和短流程。Sheet 适合轻量详情、辅助信息和不需要长时间停留的上下文查看。
...
字段很多、包含多个分组、流程包含多个步骤或用户需要专注处理复杂任务时，优先使用 full page form、独立详情页或全屏配置页。

独立详情页要求：

- 需要返回按钮。
- 返回列表时尽量保留筛选、分页、排序和滚动上下文。
- 详情 URL 应能表达资源和当前 Tab 状态。
...
- 防止重复提交。
- 成功和失败必须有反馈。
- 不显示框架默认错误、堆栈、secret、token、cookie。
...
- 可排查
- 不暴露敏感信息
- 必要时展示 request id，但低视觉权重

状态设计不能只靠颜色表达，必须结合文字、图标、说明或可访问标签。

## 12. 视觉密度

默认使用现代后台的中等信息密度：比传统重型企业后台更轻，但不能低密度到影响工作效率。
...
状态色必须稳定：成功、警告、危险、禁用、加载、空状态不能只靠颜色表达。
...
- Tablet：Sidebar 可折叠，表格保留关键列，详情进入 Sheet 或独立页。
- Mobile：表单单列，触控目标不小于 44px，表格改为关键列或紧凑列表。
- 按钮、操作区、表单行、表格操作列不得出现内部断字、换行、遮挡、异常高度或点击区过小。
...
gstack design review、Playwright 或人工 QA 必须检查：

- 页面是否符合后台系统信息密度。
- 是否存在营销页式 hero、低密度大卡片或无意义装饰。
- 表格列宽、操作列、状态列是否稳定。
- 按钮形态是否符合规则。
- 按钮是否换行、挤压或变形。
- 纯 icon 按钮是否有 `aria-label` 和 tooltip/title。
- 详情是否错误使用窄 Sheet 承载重交互。
- 表单错误是否贴近字段。
- 长文本和长 ID 是否截断、tooltip 或复制。
- 响应式下是否有遮挡、溢出、异常空白。
- console 是否有错误。
- Network 是否有非预期失败请求。
...
- Toolbar actions
- Row actions
- Button form: icon + text, icon-only, or `DropdownMenu`, with no wrapping labels
- Sheet / Dialog interactions
- Form fields
- Permission rules
- Loading / Empty / Error / Forbidden / Pending / Readonly states
- Long text / long ID handling
- Responsive fallback
...
- Button labels 是否无换行，纯 icon 按钮是否有 tooltip/aria-label
- 长文本和长 ID 是否不会撑破布局
- 表格操作列是否稳定且不改变行高
- Sheet / Dialog 交互是否可用
- 复杂交互是否没有被塞进窄 Sheet
- Empty / Loading / Error states 是否存在
- Forbidden / Pending / Readonly states 是否合理
- focus ring 是否可见
- 浏览器 console 是否有明显错误
- Network 是否有非预期失败请求
...
- 查询筛选项
- Sheet / Dialog / 表单交互
- Loading / Empty / Error / Forbidden / Pending / Readonly 状态
- 响应式降级策略
- 长文本、长 ID 和操作列处理
