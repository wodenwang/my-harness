...
- 当前模板：Ant Design。
- 默认规则：用户没有明确 UI 框架倾向时，必须选择 shadcn/ui；只有用户明确倾向 Ant Design、antd 或 Ant 时才使用本模板。
- 从零到一规则：如果这是新的后台 Admin Console，且用户没有明确风格倾向，默认采用 Ant Design Pro 作为后台框架、整体布局和页面模板参考。
- 风格规则：默认沿用 Ant Design Pro 的后台布局风格和 Ant Design 官方默认 style，不主动改 token、圆角、主色、阴影、组件间距或组件形态。
- 允许偏离：只有项目已有确认品牌规范、现有实现已经稳定使用自定义主题，或用户明确要求时，才在 theme configuration 中集中调整。
...
- Ant Design 介绍：https://ant-design.antgroup.com/docs/spec/introduce-cn
- Ant Design 设计价值观：https://ant.design/docs/spec/values-cn/
- Ant Design 设计模式概览：https://ant-design.antgroup.com/docs/spec/overview-cn
- Ant Design 色彩：https://ant-design.antgroup.com/docs/spec/colors-cn
- Ant Design 动效：https://ant-design.antgroup.com/docs/spec/motion-cn
...
Ant Design 面向复杂、频繁变化、协作成本高的企业级产品。它的核心倾向不是做视觉惊艳，而是通过稳定的组件、模板、通用概念和默认模式，降低设计和研发的重复成本，让团队在高并发业务变化中保持一致。

### 官方价值观到落地规则

| 价值观 | 本项目落地要求 |
|---|---|
| 自然 | 布局、色彩、图标和动效必须降低认知成本；交互路径按真实工作场景组织。 |
| 确定性 | 复用 Ant Design 默认组件、默认 token 和稳定页面模式；避免每页重新发明控件和布局。 |
| 意义感 | 页面目标、主操作、反馈和错误恢复必须围绕用户工作使命，不做无意义装饰。 |
| 生长性 | 页面、权限、模块和项目级组件要能随角色与功能增长而扩展。 |

### 默认视觉倾向

- 以 Ant Design 默认蓝 `#1677ff` 作为 primary 方向，除非项目已有品牌色。
- 使用系统级色板、功能色与中性色，不在页面内散落硬编码颜色。
- 信息密度偏高，适合后台人员长时间、高频处理列表、表单、审批和配置。
- 组件形态遵守官方默认尺寸、间距、圆角、边框、阴影和状态样式。
- 动效只服务于反馈、层级、空间关系和注意力引导；必须自然、高效、克制。
- 设计模式优先复用模板、基础组件、业务组件/模块和通用概念。

### Ant Design Pro 后台倾向

后台管理类项目从零到一时，页面结构、信息架构、应用壳层和工程切分默认直接沿用 Ant Design Pro 的成熟后台倾向，但不要机械复制其 demo 数据或整套脚手架。

可参考的方向：

- 使用企业应用开箱模板思路组织页面，而不是从空白页面临场拼装。
- 常见后台页面优先映射到 Dashboard、Form、List、Profile、Result、Exception、Account、User 等成熟类型。
- List 类页面优先考虑 Table List、Search List、Basic List 和 Card List 的差异：核心 CRUD 仍默认 Table List，只有对象更像应用/项目/文章时才考虑 Card List。
- Form 类页面区分 Basic Form、Step Form 和 Advanced Form：字段少用 Drawer/Form，流程性或复杂配置用 full page form / step form。
- Result 和 Exception 页面必须作为后台闭环的一部分，不把 403、404、500、提交成功、提交失败留到实现阶段再补。
- 保留 responsive、mock、i18n、测试和可主题化的工程意识，但本模板不要求直接采用 Umi 或 Ant Design Pro 的完整技术栈。

默认后台布局：

- 使用 Ant Design Pro 风格的 `Layout`：左侧 `Sider`、顶部 `Header`、内容区 `PageContainer` / 项目级 `PageHeader`。
- 一级导航在 Sider，用户菜单、通知、设置、语言或帮助入口在 Header 右侧。
- 页面内容遵循 Pro 的模板分层：页面标题区、筛选/工具区、主体表格或表单、详情/结果/异常闭环。
- Dashboard 只用于真实监控或运营总览；常规管理功能不要伪装成指标卡片堆叠。
- 初始实现可以采用轻量项目级 wrapper 复刻 Pro 的布局语义，不要求引入完整 Ant Design Pro 脚手架。

### 主题素材解析规则

如果用户提供主题色、官网、logo、截图或品牌素材，必须先解析再定主题，不要直接照搬营销页视觉。

解析步骤：

1. 提取主色、辅助色、中性色、背景色、危险/成功/警告色倾向。
2. 判断品牌气质：稳重、科技、医疗、金融、教育、消费、开发者工具等。
3. 检查色彩是否适合后台长时间使用：对比度、饱和度、浅色背景可读性、状态色冲突。
4. 映射到 Ant Design token：`colorPrimary`、功能色、page background、border、radius、font 和图表色。
5. 如果素材来自官网或营销页，只提取安全的品牌 token，不复制 hero、渐变、大图、装饰图形或营销式排版。

默认决策：

- 没有素材：使用 Ant Design Pro 默认后台布局与 Ant Design 默认蓝。
- 只有明确主题色：以该颜色作为 `colorPrimary` 候选，生成可读的浅色后台 token，并保留 Ant Design Pro 布局。
- 有 logo：从 logo 提取主色和强调色；如果 logo 色过亮或过饱和，用作 accent，不直接作为大面积背景。
- 有官网：提取品牌色、字体气质和图形语言，但后台仍保持 Pro 的信息架构和操作密度。
...
只有用户明确选择 Ant Design、antd 或 Ant 时，UI 组件体系才以 Ant Design 为主。
