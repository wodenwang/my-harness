...
5. If UI/product work is in scope, invoke or recommend `my-harness-writing-design`.
6. If deployment/release infrastructure is in scope, invoke or recommend `my-harness-writing-deployment`.
7. Finish with a `my-harness-next-action` style handoff: current SOP status, next gate, and a copyable recommended prompt.
