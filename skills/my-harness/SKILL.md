...
3. Design artifact / approved visual target: for UI work, `DESIGN.md` must exist first; explicit `my-harness-writing-design` runs must refresh the my-harness latest governance addendum in existing `DESIGN.md` while preserving project-specific content; if the system lacks an app/product title, logo, or favicon, use Creative Production `logo-explorer` to create logo/favicon/app-icon directions before finalizing prototypes; Product Design `get-context` -> `ideate` -> user selection produces at least three prototype/visual directions by default when no visual target exists; step 3 must leave an approved / selected / system-recommended accepted visual target, not just a design direction; the approved target must record its source, rationale, target mockup or reference, implementation spec extraction, layout, interaction states, responsive behavior, key pages, component inventory, shadcn component/block mapping, Tailwind token / CSS variable mapping, 8px spacing decisions, and no unapproved non-shadcn UI framework; existing screenshots, URL captures, Figma frames, current UI, or design notes may also qualify when the same implementation spec is extracted; Pencil prototype is optional for complex modules or human alignment
...
Frontend design rule: for UI, frontend, dashboard, or app work, create or verify `DESIGN.md` with `my-harness-writing-design` before design review, visual target creation, frontend planning, implementation, or design QA. When the user explicitly invokes `my-harness-writing-design` in an existing project, merge the latest my-harness governance addendum into `DESIGN.md` without overwriting project-specific design content. All design-related gates must strictly follow `DESIGN.md` and `design/`, including typography, spacing, technology stack, components, chart library, colors, responsive rules, states, and accessibility.

Product Design rule: when Product Design is installed, its focused skills replace gstack `/plan-design-review` and `/design-review` for design-related gates. Product Design creates step 3 approved visual targets through `get-context` -> `ideate` with at least three prototype/visual directions, assists frontend scaffolding with `image-to-code` / `url-to-code` inside step 7 after `IMPLEMENTATION_PLAN.md` exists, and produces visual-fidelity evidence such as `design-qa.md` before step 10. Step 10 must compare the implemented UI against the selected prototype and `DESIGN.md`, then keep fixing until the result is highly faithful or deviations are explicitly accepted. If Product Design is unavailable, do not require installation; continue with the selected design baseline, existing UI references, screenshots, or Pencil only when human alignment requires it.

Frontend Fidelity First rule: for UI-heavy projects such as CRM, Portal, Admin Console, dashboards, and apps, step 6-11 normally run as two evidence loops while the canonical 15-step ledger stays unchanged. Step 6A-11A is the Frontend Fidelity Loop: write a frontend plan that references the step 3 approved visual target and extracts implementation spec; implement the UI with mock data and high-fidelity layout/interaction as the first priority; run frontend verification; run browser screenshots before design QA; then pass a hard Product Design fidelity gate with target mockup, screenshots, differences, fixes, before/after evidence, and accepted deviations, followed by mock functional QA. Step 6B-11B is the Backend Integration Loop: write the backend/API integration plan, replace mock data with real services, rerun verification, browser checks, visual regression, and functional QA so real data does not break layout or interaction. Step 12-15 remain unchanged.
